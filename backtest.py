    ...
    async def get_candles(self, timeframe: TimeFrame, days: int = 21) -> List[HistoricCandle]:
        ...
        try:
            with Client(self.token) as client:
                ...
                # Корректируем количество часов для разных ТФ
                hours_multiplier = {
                    TimeFrame.HOUR_1: 1,
                    TimeFrame.MIN_30: 1.2,
                    TimeFrame.MIN_15: 1.5,
                    TimeFrame.MIN_5: 2
                }
                
                hours_needed = int(days * 24 * hours_multiplier[timeframe])
                from_time = to_time - timedelta(hours=hours_needed)
                
                interval = self.get_interval_for_timeframe(timeframe)
                
                logger.info(f"📡 Загрузка {timeframe.value}: {from_time.strftime('%d.%m %H:%M')} - {to_time.strftime('%d.%m %H:%M')}")
...
class MultiTimeFrameAnalyzer:
    ...
    def print_results(self, all_signals: Dict[TimeFrame, List[TimeFrameSignal]], 
        ...
        for entry in entries:
            count = entry.get_confirmation_count()
            confirmation_stats[count] = confirmation_stats.get(count, 0) + 1
        
        print(f"\n📊 РАСПРЕДЕЛЕНИЕ ПО ПОДТВЕРЖДЕНИЯМ:")
        for conf_count in sorted(confirmation_stats.keys(), reverse=True):
            count = confirmation_stats[conf_count]
            pct = (count / len(entries)) * 100
            print(f"   • {conf_count} подтверждений: {count} сигналов ({pct:.1f}%)")
        ...
        if entries:
            ...
            if hour_counts:
                ...
                print(f"   • Самый активный час: {best_hour}:00 ({max(hour_counts.values())} сигналов)")
            
            # Качество сигналов
            avg_confidence = np.mean([e.confidence_score for e in entries])
            high_quality = len([e for e in entries if e.confidence_score >= 80])
            
            print(f"   • Средняя уверенность: {avg_confidence:.1f}%")
            print(f"   • Высококачественных (>80%): {high_quality}")
        ...
        if entries:
            best = entries[0]
            print(f"🎯 ДЛЯ АВТОМАТИЧЕСКОЙ ТОРГОВЛИ:")
            print(f"   • Минимальная уверенность: 70%")
            print(f"   • Обязательно ждать подтверждения на 15м или 5м")
            print(f"   • Входить в течение 30-60 минут после сигнала")
            
            print(f"\n📈 КОД СТРАТЕГИИ (Python):")
            print("# Основные параметры")
            print("ADX_THRESHOLD = 23")
            print("EMA_PERIOD = 20") 
            print("MIN_CONFIDENCE = 70")
            print("")
            print("# Условия входа")
            print("def check_entry_signal(h1_data, m15_data):")
            print("    # 1. Основной сигнал на 1h")
            print("    if h1_adx > ADX_THRESHOLD and h1_price > h1_ema20 and h1_di_plus > h1_di_minus:")
            print("        # 2. Подтверждение на 15м в течение часа") 
            print("        if m15_adx > ADX_THRESHOLD and m15_price > m15_ema20:")
            print(f"            return True  # Входим по цене {best.entry_price:.2f}")
            print("    return False")
            
            print(f"\n🏆 ЛУЧШИЙ ИСТОРИЧЕСКИЙ СИГНАЛ:")
            print(f"   📅 Дата: {best.entry_time.strftime('%d.%m.%Y %H:%M')}")
            print(f"   💰 Цена: {best.entry_price:.2f} руб") 
            print(f"   🎯 Уверенность: {best.confidence_score:.1f}%")
            print(f"   ✅ Подтверждений: {best.get_confirmation_count()}")
            print(f"   📊 ADX: {best.main_signal.adx:.1f}")
        
        print(f"\n🔄 СЛЕДУЮЩИЕ ШАГИ:")
        print("1. Настроить уведомления для новых сигналов")
        print("2. Протестировать на бумажной торговле")
        print("3. Добавить управление рисками (стоп-лосс)")
        print("4. Оптимизировать размер позиции")
