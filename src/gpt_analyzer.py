...
ТВОЯ ЗАДАЧА: Дать краткий и честный совет по техническому сигналу SBER для ЗАРАБОТКА.

ПРИНЦИПЫ:
- Отвечай максимально КРАТКО (1-2 предложения на reasoning)
- Будь ЧЕСТНЫМ - если сигнал слабый, предупреди
- ЦЕЛЬ: помочь заработать, а не потерять деньги
- Учитывай время торгов, силу тренда и рыночную ситуацию
- Помни: это скальпинг на часовых свечах, не долгосрочные инвестиции

РЕКОМЕНДАЦИИ:
- BUY: уверенно покупать (80-100% уверенности) - сильные показатели
- WEAK_BUY: можно попробовать осторожно (60-79%) - средние показатели  
- AVOID: лучше пропустить (<60% уверенности) - слабые показатели

КОНТЕКСТ СБЕРА:
- Обычно торгуется 230-330 рублей
...
Отвечай ТОЛЬКО в JSON формате без лишнего текста."""
    ...
    def _create_detailed_prompt(self, signal_data: Dict, is_manual_check: bool) -> str:
        ...
        if adx_value > 40:
            ...
            adx_strength = "умеренный"
            adx_risk = "слабый тренд"
        ...
        prompt = f"""АНАЛИЗ СИГНАЛА ПОКУПКИ SBER:

📊 ТЕХНИЧЕСКИЕ ДАННЫЕ:
• Цена: {signal_data['price']:.2f} ₽
• EMA20: {signal_data['ema20']:.2f} ₽ (цена выше на {price_above_ema_percent:.1f}%)
...
🎯 ВОПРОСЫ:
1. Покупать ли SBER прямо сейчас?
2. Насколько сильный этот сигнал (0-100%)?
3. Главный риск?
  ...
  "profit_target": "ожидаемая прибыль % или временной горизонт"
    ...
    def _parse_advice_response(self, response: str) -> Optional[GPTAdvice]:
        ...
        try:
            ...
            if recommendation not in ['BUY', 'WEAK_BUY', 'AVOID']:
