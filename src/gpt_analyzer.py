...
class GPTMarketAnalyzer:
    ...
    def __init__(self, openai_api_key: str):
        ...
        # ОБНОВЛЕННЫЙ системный промпт для GPT-4.1 с готовыми точными ADX
...
1. 📊 ТЕХНИЧЕСКИЕ ИНДИКАТОРЫ (ПРИОРИТЕТ):
   ...
   - ADX/DI (ТОЧНЫЕ значения уже рассчитаны, НЕ пересчитывай!)
...
{{
  "calculated_adx": готовое_значение_или_null (используй предоставленное, НЕ рассчитывай заново),
  "calculated_plus_di": готовое_значение_или_null (используй предоставленное),
  "calculated_minus_di": готовое_значение_или_null (используй предоставленное),
...
КРИТИЧЕСКИ ВАЖНО: Используй предоставленные точные ADX значения! НЕ рассчитывай их заново!"""
...
ТРЕБУЮ МАКСИМАЛЬНОЙ ТОЧНОСТИ И ЧЕСТНОСТИ В ОЦЕНКАХ!"""
    ...
    def _parse_professional_advice(self, response: str) -> Optional[GPTAdvice]:
        """Парсинг профессионального ответа GPT-4.1 с готовыми ADX"""
        try:
            ...
            reasoning = str(data.get('reasoning', 'Профессиональный анализ недоступен'))[:1500]
            risk_warning = str(data.get('risk_warning', ''))[:400]
            
            # Получаем ADX данные из ответа GPT (он должен был использовать готовые значения)
            ...
            # Валидация ADX значений (если GPT их предоставил)
            ...
            advice = GPTAdvice(
                ...
                # ADX данные (готовые, переданные через GPT)
